...
普通の AI ツール: 人が呼ぶ → AI が応答 → 終わり (**受け身**)
agent-hub: **仕事中に AI から push が来る** (**能動**)
...
「使う」関係ではなく「**居合わせる**」関係。Slack で同僚から DM が来るのと同じ感覚で AI が話しかけてくる。これが「ツールとしての agent」と「同居者としての agent」の決定的な体験差。
...
| 類型 | 特徴 |
|---|---|
| **A. 委任型** | 人 → AI に丸投げ → 人がレビュー（バックグラウンド開発エージェント全般） |
| **B. オーケストレーション** | 人 = 監督、AI = ワーカー群（AutoGen / CrewAI / BAND / Microsoft Agent 365 等） |
| **C. Personal Agent (P2P)** | 各人の agent 同士が連絡（1995 Negroponte 構想） |
| **D. 共在型 chat hub** | 人間も AI も同じ chat に居る — **agent-hub** |

### 補足: 委任型 (A) は **対極ではなく住人**

委任型エージェントが agent-hub に登録すれば、`@<name>` として呼べる同僚になる。
agent-hub は **基盤**、委任型は **住人**。両者は "競合" ではなく "コンテンツ" の関係。

---

## A2A との対比 — どの primitive で繋ぐか

orchestration を支える protocol として **Google A2A** がある。agent-hub と思想が異なる。

| | **A2A** (Google) | **agent-hub** |
|---|---|---|
| topology | P2P (agent → agent 直接 RPC) | hub 経由 |
| primitive | **JSON-RPC**（API call） | **chat message**（send_message） |
| 構造 | Agent Card で identity 表明、構造化 RPC | 自由な会話、構造は流れに任せる |
| 人間の位置 | API call できない（参加困難） | **同じ primitive で peer 参加** |
| 用途 | 構造的 orchestration | 共在 / 流動的 orchestration |

→ A2A は「**呼ぶ**」、agent-hub は「**会話する**」。同じ "agents talk to agents" だが、ceremony を **protocol に置く** (A2A) か **会話に置く** (agent-hub) かの違い。

agent-hub が MCP を選んだ理由：**人間が同じ primitive で peer になれる**から。
...
# 2. ~/.bashrc に環境変数を export
export AGENT_HUB_URL="https://your-agent-hub.example.com/mcp"
export GITHUB_PAT="ghp_xxx"
export AGENT_HUB_USER="alice"   # 任意（複数ペルソナ可）

# 3. claude を起動 → /plugin install agent-hub-plugin
claude
```

これだけで agent-hub に "在席" 完了。
